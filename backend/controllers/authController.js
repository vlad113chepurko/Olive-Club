    ...
    function getTemplateWithCode(language, code) {
        const supportedLangs = ['en', 'ru', 'cn', 'sa'];
        const lang = supportedLangs.includes(language) ? language : 'en';

        const templatePath = path.join(__dirname, '..', 'templates', `confirm_code_${lang}.html`);
    ...
    const register = async (req, res) => {
        try {
            ...
            const htmlWithCode = getTemplateWithCode(language, verificationCode);
    ...
    const resendCode = async (req, res) => {
        try {
            let { email, language = 'en' } = req.body;
            email = String(email).toLowerCase();
